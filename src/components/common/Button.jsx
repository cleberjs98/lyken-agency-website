...
function Button({
  ...
  const classes = [
    "lyken-button lyken-text-button relative inline-flex min-h-11 max-w-full min-w-0 items-center justify-center overflow-hidden rounded-none border px-4 py-4 text-center leading-tight transition-colors duration-200 active:opacity-90 focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-lyken-gold disabled:pointer-events-none disabled:cursor-not-allowed disabled:border-lyken-gold-line-20 disabled:text-lyken-text-soft disabled:opacity-55 sm:px-6",
  ...
  if (href) {
    return (
      <motion.a
        ...
        {...props}
      >
        <span className="relative z-10 min-w-0 max-w-full whitespace-normal break-words [overflow-wrap:anywhere]">
          {children}
        </span>
